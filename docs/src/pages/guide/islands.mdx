...
[previous section]: /guide/islands
...
<NuxtLogo/> Nuxt, on the contrary, follows an architecture where <kbd>static îslands</kbd> are embedded within a dynamic app. Marking components as [server components] are optional.
...
In the following example from [The Vue Point in îles][demo] ([repo][blog repo]), the `CatDisplay` îsland is loaded from the parent component and hydrated using the `client:load` directive.
