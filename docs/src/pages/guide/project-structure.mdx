...
> Use the `autoImport` key in `iles.config.ts` to customize [Auto Import][unplugin-auto-import] configurations.
...
> Use the `components` key in `iles.config.ts` to customize [Auto Import][unplugin-vue-components] configurations for components.
